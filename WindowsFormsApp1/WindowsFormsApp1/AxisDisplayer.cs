...
namespace 抛物线模拟
{
    public partial class AxisDisplayer :UserControl
    {
        ...
        public double V=1, Arg=45;
        public double StuffX=4,StuffY=2,SWidth=0.3;
        ...
        protected void RenderPWX(Graphics graphics)
        {
            graphics.DrawLine(new Pen(Color_Black,3), new Point((int)MathToPixelX(StuffX - SWidth/2), (int)MathToPixelY(StuffY)), 
                new Point((int)MathToPixelX(StuffX + SWidth/2), (int)MathToPixelY(StuffY)));
            var realarg=Arg/180*Math.PI;
            var y = 0d;
            var x = 0d;
            var xV=V*Math.Cos(realarg);
            var yV=V*Math.Sin(realarg);
            ...
            for(; ; )//ms
            {
                ...
                y += yV / 1000;
                yV -= 9.8 / 1000;
                var newp= new Point((int)MathToPixelX(x), (int)MathToPixelY(y));
                if (newp != last)
                {
                    ps.Add(newp);
                    last = newp;
                ...
                if (ps.Count > 100)
                {
                    graphics.DrawLines(Pen_Black, ps.ToArray());
                    ps.Clear();
                }
                if (y < 0)
                    break;
